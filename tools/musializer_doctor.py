...
def _existing_application(root: Path) -> Optional[Path]:
    candidates = (
        root / "build/musializer",
        root / "build/musializer.exe",
        root / "musializer",
        root / "musializer.exe",
        root / "build/Musializer.app/Contents/MacOS/Musializer",
    )
...
def audit(*, root: Path = ROOT, analysis_dir: Optional[Path] = None,
    ...
    checks.append(_check(
        ...
        detail=str(application) if application else "run ./nob build release",
    ))
